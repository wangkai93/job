...
<TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%"
		...
		<TBODY>
			<TR>
				<TD class=head colspan="2"><SPAN class=TAG>招聘信息</SPAN></TD>
			</TR>
			<TR>
						...
						<TBODY>
							<c:if test="${! empty jobsList }">
								<c:forEach items="${jobsList }" var="job">
									<TR>
										<TD align=left><A href="${pageContext.request.contextPath }/job/show/${job.id}">${job.need }</A></TD>
										<TD align=center>${job.num }人</TD>
										<TD align=center>${job.danwei }</TD>
										<TD align=center>${job.tel }</TD>
										<TD align=center>${job.place }</TD>
										<TD align=right>${job.addtime }</TD>
									</TR>
								</c:forEach>
							</c:if>
						</TBODY>
